...
SELECT count(*) FROM (SELECT first_name, last_name, country, last_connection FROM users WHERE country = 'Iceland') as subquery WHERE last_connection LIKE ('112.%')


-- 6. Listado de usuarios de Iceland, tienen dirección IP
-- que inicia en 112 ó 28 ó 188
-- Tip: Agrupar condiciones entre paréntesis 
SELECT * FROM (SELECT first_name, last_name, country, last_connection FROM users WHERE country = 'Iceland') as subquery WHERE last_connection LIKE ('112.%') OR last_connection LIKE('28.%') OR last_connection LIKE('188.%')


-- 7. Ordene el resultado anterior, por apellido (last_name) ascendente
-- y luego el first_name ascendentemente también
SELECT * FROM (SELECT first_name, last_name, country, last_connection FROM users WHERE country = 'Iceland') as subquery WHERE last_connection LIKE ('112.%') OR last_connection LIKE('28.%') OR last_connection LIKE('188.%') ORDER BY last_name ASC, first_name ASC
...
SELECT country, COUNT(*) FROM users WHERE country IN ('Mexico', 'Honduras', 'Costa Rica')
GROUP BY country
